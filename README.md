...
# HOW IT WORKS
Each value stored is *XOR* obfuscated with a generated key, as its pointer and its specifications values (size, offset, etc).\
The stored address is not directly the one pointing to the good memory buffer, and the obfuscated value has also pre and post noise data padding.\
